...
/* =========================================================
   AI AGENT SETTINGS
   Paste your own Google Gemini API key here (free — get one
   at aistudio.google.com/apikey, no card required) to enable
...
// can't run up an unexpected bill. Raise this once you know your
// normal daily volume.
const MAX_DAILY_AI_MESSAGES = 150;
...
function doGet(e) {
  ...
  if (action === 'listLeadsAdmin') {
    if (e.parameter.key !== SECRET_KEY) return respond({ error: 'unauthorized' });
    return respond(listLeads());
  }
...
function doPost(e) {
  ...
  if (action === 'logLead') {
    // No key check — this is called by site visitors submitting enquiries
    return respond(logLead(body));
  }
  if (action === 'updateLeadStatus') {
    if (body.key !== SECRET_KEY) return respond({ error: 'unauthorized' });
    return respond(updateLeadStatus(body.id, body.status));
  }
  if (action === 'chatWithAgent') {
    // Public — this is the visitor-facing chat widget
    return respond(chatWithAgent(body));
  }
  return respond({ error: 'unknown action' });
}
...
/* =========================================================
   LEAD TRACKING
   Every enquiry form on the site (contact, hotel cart,
   flights, visa, passport, custom package requests) calls
   logLead() alongside opening WhatsApp — so even if a
   WhatsApp message gets missed, nothing is lost. Also sends
   an email to OWNER_EMAIL the moment each one comes in.
   ========================================================= */

const LEAD_HEADERS = ['id', 'timestamp', 'type', 'name', 'phone', 'details', 'status'];

function logLead(body) {
  const sheet = getSheetGeneric('Leads', LEAD_HEADERS);
  const id = Utilities.getUuid();
  const now = new Date().toISOString();
  sheet.appendRow([
    id, now, body.type || 'general', body.name || '', body.phone || '',
    body.details || '', 'New'
  ]);
  sendLeadEmail(body.type, body.name, body.phone, body.details);
  return { success: true, id: id };
}

function sendLeadEmail(type, name, phone, details) {
  try {
    MailApp.sendEmail({
      to: OWNER_EMAIL,
      subject: 'New ' + (type || 'website') + ' enquiry — ' + (name || 'unknown'),
      body: 'New enquiry received on your website:\n\n' +
        'Type: ' + type + '\n' +
        'Name: ' + name + '\n' +
        'Phone: ' + phone + '\n\n' +
        'Details:\n' + details + '\n\n' +
        '— Dharti Holidays website'
    });
  } catch (e) {
    // Don't let an email failure break lead logging
  }
}

// Run this manually from the Apps Script editor (select it in the
// function dropdown next to Run) to force the email-permission
// prompt and confirm email sending works, independent of the web app.
function testEmailSetup() {
  MailApp.sendEmail({
    to: OWNER_EMAIL,
    subject: 'Test email from Dharti Holidays website',
    body: 'If you are reading this, email alerts are working correctly.'
  });
}

function listLeads() {
  const sheet = getSheetGeneric('Leads', LEAD_HEADERS);
  const rows = rowsToObjects(sheet, LEAD_HEADERS);
...
function updateLeadStatus(id, status) {
  ...
  const rowIdx = findRowIndexById(sheet, id);
  if (rowIdx === -1) return { error: 'not found' };
  sheet.getRange(rowIdx, 7).setValue(status); // column 7 = status
  return { success: true };
}

/* =========================================================
   AI AGENT — catalog-aware chat assistant
   Answers visitor questions using real destinations, packages,
   visa/passport services (given directly in the system prompt)
   and hotels (looked up live via a tool, since there can be
   thousands — too many to paste into every prompt). Can also
   save a lead once it has a name + phone + what they want.
   ========================================================= */

function getTodayCountKey() {
  const today = Utilities.formatDate(new Date(), Session.getScriptTimeZone() || 'Etc/UTC', 'yyyy-MM-dd');
  return 'ai_chat_count_' + today;
}

function checkAndIncrementDailyCount() {
  const props = PropertiesService.getScriptProperties();
  const key = getTodayCountKey();
  const current = Number(props.getProperty(key) || '0');
  if (current >= MAX_DAILY_AI_MESSAGES) return false;
  props.setProperty(key, String(current + 1));
  return true;
}

function buildAgentSystemPrompt() {
  const destinations = listDestinations(true);
  const packages = listPackages(true);
  const visaServices = listVisaServices(true);
  const passportServices = listPassportServices(true);

  const destText = destinations.map(d => `- ${d.name} (${d.region}): ${d.tagline}`).join('\n') || 'None listed yet.';
  const pkgText = packages.map(p => {
    const route = (p.routeFrom && p.routeTo) ? `${p.routeFrom} to ${p.routeTo}` : p.destinationLabel;
    return `- [${p.category}] ${route} (${p.destinationLabel}): ${p.description} — ${p.meta1Label}: ${p.meta1Value}, Price: ${p.price} ${p.priceUnit}`;
  }).join('\n') || 'None listed yet.';
  const visaText = visaServices.map(v => `- ${v.country} ${v.visaType}: processing ${v.processingTime}, price ${v.price} ${v.priceUnit}`).join('\n') || 'None listed yet.';
  const passportText = passportServices.map(p => `- ${p.serviceType}: processing ${p.processingTime}, price ${p.price} ${p.priceUnit}`).join('\n') || 'None listed yet.';

  return `You are the helpful AI travel assistant for Dharti Holidays, a travel agency in Ahmedabad, Gujarat, India.

Business contact: Phone +91 93746 15071 / +91 98797 52230, WhatsApp +91 98240 44070, Email dhartiholidays@gmail.com, Address: 521, Satya-2, Near Bharat Petrol Pump, Ranna Park, Naranpura, Ahmedabad 380013.

Your job: answer visitor questions about destinations, packages, hotels, visas, and passport services using ONLY the real data given below and the search_hotels tool.

Rules:
- Never invent prices, availability, or details that aren't in the data below or returned by a tool.
- For flights: there is no live fare data on this site — direct visitors to the Flights page or WhatsApp for a personal quote, don't guess prices.
- For hotel questions, always use the search_hotels tool rather than guessing — even if you think you know.
- If a visitor shows real intent to book or wants a callback, ask for their name and phone number. Only call capture_lead once you actually have both, plus a short summary of what they're interested in. Never call it without the user actually giving you their name and phone.
- For anything outside this data, or as a natural next step, invite them to continue on WhatsApp: https://wa.me/919824044070
- Keep replies short and conversational (2-4 sentences), matching a warm, professional travel-desk tone. Don't write long essays.
...
CURRENT PASSPORT SERVICES:
${passportText}`;
}

function executeSearchHotels(input) {
  const rows = getRows('approved');
  const destName = (input.destination || '').toLowerCase();
  let matches = rows.filter(r => r.destinationName && r.destinationName.toLowerCase().includes(destName));
  if (input.category) matches = matches.filter(r => r.category === input.category);
  if (input.maxPrice) matches = matches.filter(r => Number(r.price) <= Number(input.maxPrice));
  const trimmed = matches.slice(0, 5).map(r => ({
    name: r.hotelName, location: r.location, category: r.category, price: r.price, rating: r.rating
  }));
  return { count: trimmed.length, hotels: trimmed };
}

function executeCaptureLead(input) {
  logLead({ type: 'ai-chat', name: input.name, phone: input.phone, details: input.interest });
  return { success: true };
}

function callGeminiApi(contents, toolDeclarations) {
  const payload = {
    contents: contents,
...
function chatWithAgent(body) {
  try {
    return chatWithAgentInner(body);
  } catch (err) {
    // Surface the real error directly instead of a generic failed execution —
    // this is what lets us see exactly what broke from the chat widget itself.
    return { error: 'exception', detail: err.toString() };
  }
}

function chatWithAgentInner(body) {
  ...
  const tools = [
    {
      name: 'search_hotels',
      description: 'Search approved hotels by destination name, with optional category and max price per night filters.',
      parameters: {
        type: 'object',
        properties: {
          destination: { type: 'string', description: 'Destination name, e.g. Kerala' },
          category: { type: 'string', description: 'Budget, Deluxe, or Luxury (optional)' },
          maxPrice: { type: 'number', description: 'Maximum price per night in INR (optional)' }
        },
    ...
    {
      name: 'capture_lead',
      description: 'Save the visitor as a lead once you have their name, phone number, and know what they are interested in.',
      parameters: {
        type: 'object',
        properties: {
          name: { type: 'string' },
          phone: { type: 'string' },
          interest: { type: 'string', description: 'Short summary of what they want' }
        },
  ...
  let contents = (body.history || []).concat([{ role: 'user', parts: [{ text: body.message }] }]);
  let iterations = 0;

  while (iterations < 4) {
    iterations++;
    const result = callGeminiApi(contents, tools);

    if (result.error) {
      return { error: 'api_error', detail: (result.error && result.error.message) || JSON.stringify(result.error) };
    }
    if (!result.candidates || result.candidates.length === 0) {
      return { error: 'api_error', detail: 'No candidates returned. Full response: ' + JSON.stringify(result).slice(0, 500) };
    }

    const candidate = result.candidates[0];
    if (!candidate.content || !candidate.content.parts) {
      return { error: 'api_error', detail: 'Model returned no content (finishReason: ' + (candidate.finishReason || 'unknown') + ')' };
    }
    ...
    if (functionCallParts.length > 0) {
      ...
      const responseParts = functionCallParts.map(p => {
        let output;
        if (p.functionCall.name === 'search_hotels') output = executeSearchHotels(p.functionCall.args);
        if (p.functionCall.name === 'capture_lead') output = executeCaptureLead(p.functionCall.args);
    ...
    contents.push({ role: 'model', parts: candidateParts });
    return { reply: textPart ? textPart.text : "Sorry, I couldn't quite get that — could you try again?", history: contents };
  }
