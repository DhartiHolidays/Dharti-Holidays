...
(function () {
  const widgetHtml = `
    <div class="ai-greeting-bubble" id="aiGreetingBubble">
      <button class="greeting-close" id="aiGreetingClose" aria-label="Dismiss">✕</button>
      <span class="greeting-label">Dharti AI</span>
      <p>👋 Planning a trip? Ask me about destinations, hotels & packages — instantly.</p>
    </div>
    <button class="ai-chat-toggle" id="aiChatToggle" aria-label="Chat with Dharti AI">
      💬
  ...
  function initAiChatWidget() {
    ...
    const greetingBubble = document.getElementById('aiGreetingBubble');
    const greetingClose = document.getElementById('aiGreetingClose');

    let history = [];
    let opened = false;

    // ---------- Proactive greeting bubble ----------
    // Shows once per browser session, a couple seconds after the page
    // loads, to actually draw the eye instead of waiting for someone
    // to notice a static icon on their own.
    ...
    if (!sessionStorage.getItem('aiGreetingShown')) {
      setTimeout(() => {
        if (!opened) greetingBubble.classList.add('show');
      }, 2500);
    ...
    function openChat() {
      ...
      toggle.classList.add('pulse-stopped');
      if (!opened) {
        opened = true;
    ...
    async function sendMessage() {
      ...
      try {
        ...
        if (result.error === 'not_configured') {
          ...
          if (result.detail) {
            addSystemNote('<span style="font-family:monospace; font-size:10.5px; opacity:0.8;">Debug: ' + result.detail + '</span>');
          }
        } else {
