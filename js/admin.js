...
function buildVoucherHtml() {
  ...
  const typeLabel = { flight: 'Flight', hotel: 'Hotel', package: 'Package' }[voucherType];

  let detailsHtml = '';

  if (voucherType === 'flight') {
    const airline = document.getElementById('fAirline').value.trim();
    const flightNo = document.getElementById('fFlightNo').value.trim();
    const from = document.getElementById('fFrom').value.trim();
    const to = document.getElementById('fTo').value.trim();
    const departDate = formatDate(document.getElementById('fDepartDate').value);
    const departTime = formatTime(document.getElementById('fDepartTime').value);
    const arriveDate = formatDate(document.getElementById('fArriveDate').value);
    const arriveTime = formatTime(document.getElementById('fArriveTime').value);
    const flightClass = document.getElementById('fClass').value.trim();
    const baggage = document.getElementById('fBaggage').value.trim();

    detailsHtml = `
      <div class="voucher-route-block">
        <div class="voucher-route-city">${from || '—'}<small>${departDate} ${departTime}</small></div>
        <div class="voucher-route-arrow">✈</div>
        <div class="voucher-route-city">${to || '—'}<small>${arriveDate} ${arriveTime}</small></div>
      </div>
      <div class="voucher-section">
        <h4>Flight Information</h4>
        <div class="voucher-detail-grid">
          <div><span>Airline</span><strong>${airline || '—'}</strong></div>
          <div><span>Flight Number</span><strong>${flightNo || '—'}</strong></div>
          <div><span>Class</span><strong>${flightClass || '—'}</strong></div>
          <div><span>Baggage</span><strong>${baggage || '—'}</strong></div>
        </div>
      </div>`;
  }
  ...
  const statusClass = paymentStatus.replace(' ', '-');
  ...
  return `
    <div class="voucher-doc">
      ...
      <div class="voucher-doc-body">
        ...
        ${notes ? `<div class="voucher-section"><h4>Notes</h4><p>${notes}</p></div>` : ''}
      </div>
