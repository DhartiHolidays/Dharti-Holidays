...
document.querySelectorAll('.admin-tab-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    document.getElementById('itineraryPanel').hidden = tab !== 'itinerary';
    document.getElementById('trayPanel').hidden = tab !== 'tray';
    if (tab === 'tray') renderTrayList();
...
document.getElementById('addVoucherToTrayBtn').addEventListener('click', () => {
  const html = document.getElementById('voucherPreview').innerHTML;
  if (!html) return;
  const bookingRef = document.getElementById('vBookingRef').value.trim();
  const typeLabel = { flight: 'Flight', hotel: 'Hotel', train: 'Train', package: 'Package' }[voucherType] || 'Voucher';
  addToTray('Voucher', `${typeLabel} Voucher${bookingRef ? ' — ' + bookingRef : ''}`, html);
});
...
let selectedItineraryFile = null;

document.getElementById('itineraryPdfInput').addEventListener('change', (e) => {
  selectedItineraryFile = e.target.files[0] || null;
  document.getElementById('extractItineraryBtn').disabled = !selectedItineraryFile;
  document.getElementById('itineraryStatus').innerHTML = '';
});
...
document.getElementById('extractItineraryBtn').addEventListener('click', async () => {
  if (!selectedItineraryFile) return;
  const btn = document.getElementById('extractItineraryBtn');
  const statusEl = document.getElementById('itineraryStatus');
  btn.disabled = true;
  btn.textContent = 'Reading PDF...';
  statusEl.innerHTML = '<p style="color:var(--slate); font-size:13px;">Uploading and extracting — this can take 10-20 seconds for longer itineraries.</p>';

  try {
    const base64 = await fileToBase64(selectedItineraryFile);
    const res = await fetch(APPS_SCRIPT_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'text/plain;charset=utf-8' },
      body: JSON.stringify({ action: 'extractItinerary', key: adminKey, pdfBase64: base64 }),
    });
    const result = await res.json();

    if (result.error === 'not_configured') {
      statusEl.innerHTML = '<p style="color:var(--red); font-size:13px;">The AI connection isn\'t set up yet — this needs the same Gemini API key as your chat widget.</p>';
    } else if (result.error) {
      statusEl.innerHTML = `<p style="color:var(--red); font-size:13px;">Could not extract this PDF: ${result.detail || result.error}</p>`;
    } else if (result.success) {
      statusEl.innerHTML = '<p style="color:var(--navy); font-size:13px; font-weight:700;">✓ Extracted — please review and fix anything below before generating.</p>';
      populateItineraryReview(result.itinerary);
    }
  } catch (err) {
    statusEl.innerHTML = '<p style="color:var(--red); font-size:13px;">Could not reach the server. Please try again.</p>';
    console.error(err);
  } finally {
    btn.disabled = false;
...
let rvRouteCount = 0, rvHotelCount = 0, rvDayCount = 0;

function addRouteRow(city = '', nights = '') {
  rvRouteCount++;
  const row = document.createElement('div');
  row.className = 'repeat-row';
  row.style.gridTemplateColumns = '2fr 1fr auto';
  row.innerHTML = `
    <div class="field"><label>City</label><input type="text" class="rv-route-city" value="${escapeAttr(city)}"></div>
    <div class="field"><label>Nights</label><input type="number" class="rv-route-nights" min="0" value="${escapeAttr(nights)}"></div>
    <button type="button" class="repeat-row-remove" aria-label="Remove">✕</button>`;
  row.querySelector('.repeat-row-remove').addEventListener('click', () => row.remove());
  document.getElementById('rvRouteRows').appendChild(row);
}

function addHotelRow(h = {}) {
  rvHotelCount++;
  const row = document.createElement('div');
  row.className = 'leg-block';
  row.innerHTML = `
    <div class="form-row">
      <div class="field"><label>Nights (dates)</label><input type="text" class="rv-hotel-nights" value="${escapeAttr(h.nightsLabel)}"></div>
      <div class="field"><label>City</label><input type="text" class="rv-hotel-city" value="${escapeAttr(h.city)}"></div>
    </div>
    <div class="form-row">
      <div class="field full"><label>Hotel Name</label><input type="text" class="rv-hotel-name" value="${escapeAttr(h.name)}"></div>
    </div>
    <div class="form-row">
      <div class="field"><label>Category / Stars</label><input type="text" class="rv-hotel-category" value="${escapeAttr(h.category)}"></div>
      <div class="field"><label>Meal Plan</label><input type="text" class="rv-hotel-meal" value="${escapeAttr(h.mealPlan)}"></div>
      <div class="field"><label>Room</label><input type="text" class="rv-hotel-room" value="${escapeAttr(h.room)}"></div>
    </div>
    <button type="button" class="repeat-row-remove" aria-label="Remove" style="margin-top:6px;">✕ Remove Hotel</button>`;
  row.querySelector('.repeat-row-remove').addEventListener('click', () => row.remove());
  document.getElementById('rvHotelRows').appendChild(row);
}
...
function addDayRow(d = {}) {
  ...
  row.innerHTML = `
    <div class="form-row">
      <div class="field"><label>Day Label</label><input type="text" class="rv-day-label" value="${escapeAttr(d.dayLabel)}" placeholder="Day 1"></div>
      <div class="field"><label>Date</label><input type="text" class="rv-day-date" value="${escapeAttr(d.date)}" placeholder="Sat, 14 Nov"></div>
    </div>
    <div class="form-row">
      <div class="field full"><label>Title</label><input type="text" class="rv-day-title" value="${escapeAttr(d.title)}"></div>
    </div>
    <div class="form-row">
      <div class="field full"><label>Description</label><textarea class="rv-day-desc">${escapeHtml(d.description)}</textarea></div>
    </div>
    <div class="form-row">
      <div class="field"><label>Tags (comma separated)</label><input type="text" class="rv-day-tags" value="${escapeAttr((d.tags || []).join(', '))}"></div>
      <div class="field" style="justify-content:flex-end; flex-direction:row; align-items:center; gap:8px;">
        <label style="margin-bottom:0;"><input type="checkbox" class="rv-day-noguide" ${d.noGuide ? 'checked' : ''}> No Guide This Day</label>
      </div>
    </div>
    <button type="button" class="repeat-row-remove" aria-label="Remove" style="margin-top:6px;">✕ Remove Day</button>`;
  row.querySelector('.repeat-row-remove').addEventListener('click', () => row.remove());
  document.getElementById('rvDayRows').appendChild(row);
}

function escapeAttr(val) { return (val === undefined || val === null) ? '' : String(val).replace(/"/g, '&quot;'); }
function escapeHtml(val) { return (val === undefined || val === null) ? '' : String(val).replace(/</g, '&lt;').replace(/>/g, '&gt;'); }

function populateItineraryReview(data) {
  document.getElementById('rvTripId').value = data.tripId || '';
  document.getElementById('rvDestination').value = data.destination || '';
  document.getElementById('rvStartDate').value = data.startDate || '';
  document.getElementById('rvDuration').value = data.duration || '';
  document.getElementById('rvPax').value = data.pax || '';
  document.getElementById('rvPricePerPerson').value = data.pricePerPerson || '';
  document.getElementById('rvPriceUnit').value = data.priceUnit || '';
  document.getElementById('rvTotalPrice').value = data.totalPrice || '';
  document.getElementById('rvNotes').value = data.notes || '';

  document.getElementById('rvRouteRows').innerHTML = '';
  document.getElementById('rvHotelRows').innerHTML = '';
  document.getElementById('rvDayRows').innerHTML = '';
  (data.route || []).forEach(r => addRouteRow(r.city, r.nights));
  (data.hotels || []).forEach(h => addHotelRow(h));
  (data.days || []).forEach(d => addDayRow(d));

  document.getElementById('itineraryReviewSection').hidden = false;
  document.getElementById('itineraryReviewSection').scrollIntoView({ behavior: 'smooth', block: 'start' });
}

document.getElementById('rvAddRouteBtn').addEventListener('click', () => addRouteRow());
document.getElementById('rvAddHotelBtn').addEventListener('click', () => addHotelRow());
document.getElementById('rvAddDayBtn').addEventListener('click', () => addDayRow());

function collectItineraryFromReview() {
  const route = Array.from(document.querySelectorAll('#rvRouteRows .repeat-row')).map(row => ({
    city: row.querySelector('.rv-route-city').value.trim(),
    nights: Number(row.querySelector('.rv-route-nights').value) || 0,
  })).filter(r => r.city);

  const hotels = Array.from(document.querySelectorAll('#rvHotelRows .leg-block')).map(row => ({
    nightsLabel: row.querySelector('.rv-hotel-nights').value.trim(),
    city: row.querySelector('.rv-hotel-city').value.trim(),
    name: row.querySelector('.rv-hotel-name').value.trim(),
    category: row.querySelector('.rv-hotel-category').value.trim(),
    mealPlan: row.querySelector('.rv-hotel-meal').value.trim(),
    room: row.querySelector('.rv-hotel-room').value.trim(),
  })).filter(h => h.name);

  const days = Array.from(document.querySelectorAll('#rvDayRows .leg-block')).map(row => ({
    dayLabel: row.querySelector('.rv-day-label').value.trim(),
    date: row.querySelector('.rv-day-date').value.trim(),
    title: row.querySelector('.rv-day-title').value.trim(),
    description: row.querySelector('.rv-day-desc').value.trim(),
    tags: row.querySelector('.rv-day-tags').value.split(',').map(t => t.trim()).filter(Boolean),
    noGuide: row.querySelector('.rv-day-noguide').checked,
  })).filter(d => d.title || d.description);

  return {
    tripId: document.getElementById('rvTripId').value.trim(),
    destination: document.getElementById('rvDestination').value.trim(),
    startDate: document.getElementById('rvStartDate').value.trim(),
    duration: document.getElementById('rvDuration').value.trim(),
    pax: document.getElementById('rvPax').value.trim(),
    route, hotels, days,
    pricePerPerson: document.getElementById('rvPricePerPerson').value.trim(),
    priceUnit: document.getElementById('rvPriceUnit').value.trim(),
    totalPrice: document.getElementById('rvTotalPrice').value.trim(),
    notes: document.getElementById('rvNotes').value.trim(),
  };
}

document.getElementById('generateFromReviewBtn').addEventListener('click', () => {
  const data = collectItineraryFromReview();
  renderItineraryDoc(data);
  document.getElementById('itineraryReviewSection').hidden = true;
});

function renderItineraryDoc(data) {
  const routeHtml = (data.route || []).map((r, i, arr) => {
    const arrow = i < arr.length - 1 ? '<div class="itin-route-arrow">→</div>' : '';
    return `<div class="itin-route-city">${r.city}<small>${r.nights} Night${r.nights == 1 ? '' : 's'}</small></div>${arrow}`;
  }).join('');
  ...
  const hotelRows = (data.hotels || []).map(h => `
    <tr>
      <td>${h.nightsLabel || ''}</td>
      <td>${h.city || ''}</td>
      <td><strong>${h.name || ''}</strong><span class="itin-star">${h.category || ''}</span></td>
      <td><span class="itin-meal-tag">${h.mealPlan || ''}</span></td>
      <td>${h.room || ''}</td>
    </tr>`).join('');

  const dayCards = (data.days || []).map(d => {
    const tags = (d.tags || []).map(t => `<span>${t}</span>`).join('');
    const noGuideTag = d.noGuide ? '<span class="itin-no-guide">No Guide</span>' : '';
    return `
      <div class="itin-day-card">
        <div class="itin-day-head"><span class="day-title">${d.dayLabel || ''} — ${d.title || ''}</span><span class="day-date">${d.date || ''}</span></div>
        <div class="itin-day-body">
          <p>${d.description || ''}</p>
          <div class="itin-day-tags">${tags}${noGuideTag}</div>
        </div>
      </div>`;
  }).join('');
  ...
  const html = `
    <div class="itin-doc">
      <div class="itin-doc-head">
        <div class="itin-doc-head-brand">
          <img src="images/logo.jpg" alt="Dharti Holidays">
          <div><strong>Dharti Holidays</strong><span>Ahmedabad · Travel Desk</span></div>
        </div>
        <div class="itin-doc-head-right">
          521, Satya-2, Ranna Park, Naranpura, Ahmedabad<br>
          +91 98797 52230 · +91 93746 15071<br>
          dhartiholidays@gmail.com
        </div>
      </div>

      <div class="itin-intro">
        <p>Thanks for your query — we're pleased to share your personalised ${data.destination || ''} itinerary below.</p>
        ${data.notes ? `<div class="itin-note">${data.notes}</div>` : ''}
      </div>

      <div class="itin-section">
        <div class="itin-section-title">Package Overview</div>
        <div class="itin-overview">
          <div class="itin-overview-top">
            <div class="trip-id">Trip Reference<strong>${data.tripId ? '#' + data.tripId : '—'}</strong></div>
            <div class="status-tag">Quote Prepared</div>
          </div>
          ${routeHtml ? `<div class="itin-route-strip">${routeHtml}</div>` : ''}
          <div class="itin-overview-grid">
            <div><span>Start Date</span><strong>${data.startDate || '—'}</strong></div>
            <div><span>Duration</span><strong>${data.duration || '—'}</strong></div>
            <div><span>Travellers</span><strong>${data.pax || '—'}</strong></div>
            <div><span>Destination</span><strong>${data.destination || '—'}</strong></div>
          </div>
        </div>
      </div>

      ${hotelRows ? `
      <div class="itin-section" style="padding-top:0;">
        <div class="itin-section-title">Hotels</div>
        <table class="itin-hotel-table">
          <thead><tr><th>Nights</th><th>City</th><th>Hotel</th><th>Meal Plan</th><th>Room</th></tr></thead>
      ...
      ${dayCards ? `
      <div class="itin-section" style="padding-top:0;">
        <div class="itin-section-title">Day-Wise Itinerary</div>
        ${dayCards}
      </div>` : ''}

      ${(data.pricePerPerson || data.totalPrice) ? `
      <div class="itin-section" style="padding-top:0;">
        <div class="itin-section-title">Package Cost</div>
        <div class="itin-price-panel">
          <div>
            <div class="price-label">${data.priceUnit || 'Per Person'}</div>
            <div class="price-amount">${data.pricePerPerson || ''}</div>
          </div>
      ...
      <div class="itin-doc-foot">
        <div class="itin-thank-you">Thank you for choosing Dharti Holidays</div>
        <p><strong>Dharti Holidays</strong> · 521, Satya-2, Near Bharat Petrol Pump, Ranna Park, Naranpura, Ahmedabad 380013</p>
        <p>+91 98797 52230 · +91 93746 15071 · dhartiholidays@gmail.com · www.dhartiholidays.com</p>
      </div>
    </div>`;
  ...
  preview.hidden = false;
  document.getElementById('itineraryActions').hidden = false;
  preview.scrollIntoView({ behavior: 'smooth', block: 'start' });
}
...
document.getElementById('newItineraryBtn').addEventListener('click', () => {
  selectedItineraryFile = null;
  document.getElementById('itineraryPdfInput').value = '';
  document.getElementById('extractItineraryBtn').disabled = true;
  document.getElementById('itineraryStatus').innerHTML = '';
  document.getElementById('itineraryPreview').hidden = true;
  document.getElementById('itineraryPreview').innerHTML = '';
  document.getElementById('itineraryActions').hidden = true;
  document.getElementById('itineraryReviewSection').hidden = true;
  document.getElementById('rvRouteRows').innerHTML = '';
  document.getElementById('rvHotelRows').innerHTML = '';
  document.getElementById('rvDayRows').innerHTML = '';
...
function addToTray(type, label, html) {
  trayIdCounter++;
  documentsTray.push({ id: trayIdCounter, type, label, html, selected: true });
  alert(`Added to Documents Tray: ${label}\n\nGo to the "Documents Tray" tab to print it alone or combined with others.`);
}

document.getElementById('addItineraryToTrayBtn').addEventListener('click', () => {
  const html = document.getElementById('itineraryPreview').innerHTML;
  if (!html) return;
  const dest = document.getElementById('rvDestination') ? document.getElementById('rvDestination').value : '';
  addToTray('Itinerary', `Itinerary — ${dest || 'Untitled'}`, html);
});

function renderTrayList() {
  const list = document.getElementById('trayList');
  const empty = document.getElementById('trayEmptyState');
  const actions = document.getElementById('trayActions');
  list.innerHTML = '';

  if (documentsTray.length === 0) {
    empty.hidden = false;
    actions.hidden = true;
    return;
  }
  ...
  documentsTray.forEach(item => {
    const row = document.createElement('div');
    row.className = 'tray-item';
    row.innerHTML = `
      <div class="tray-item-left">
        <input type="checkbox" ${item.selected ? 'checked' : ''}>
        <span class="tray-item-type-tag">${item.type}</span>
        <span class="tray-item-label">${item.label}</span>
      </div>
      <button class="tray-item-remove" aria-label="Remove">✕</button>`;
    row.querySelector('input[type="checkbox"]').addEventListener('change', (e) => {
      item.selected = e.target.checked;
    });
    row.querySelector('.tray-item-remove').addEventListener('click', () => {
      documentsTray = documentsTray.filter(d => d.id !== item.id);
      renderTrayList();
    });
...
document.getElementById('printTrayBtn').addEventListener('click', () => {
  const selected = documentsTray.filter(d => d.selected);
  if (selected.length === 0) {
    alert('Select at least one document to print.');
    return;
  }
  const preview = document.getElementById('trayPreview');
  preview.innerHTML = selected.map(d => `<div class="tray-print-item">${d.html}</div>`).join('');
  preview.hidden = false;
  window.print();
});

document.getElementById('clearTrayBtn').addEventListener('click', () => {
  if (confirm('Remove all documents from the tray?')) {
    documentsTray = [];
    renderTrayList();
  }
});
